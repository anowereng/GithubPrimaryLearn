using System;
using System.Threading.Tasks;
using Sampan;
using UdemyCoreCourse.Models;
using System.Data;
using System.Collections.Generic;
using System.Collections;
namespace UdemyCoreCourse.Data
{
    public class AuthRepository 
    {
        public Task<User> Login(string username, string password)
        {
            // var user = a
            var user = await 
        }

        public async Task<User> Register(User user, string password)
        {
            byte[] passwordHash, passwordsalt;
            CreatePasswordHash(password,out passwordHash, out passwordsalt);
            user.PasswordHash=passwordHash;
            user.PasswordSalt=passwordsalt;
            await prcRegisterUser(user);
            return user;
        }

        private void CreatePasswordHash(string password, out byte[] passwordHash, out byte[] passwordsalt)
        {
           using(var hmac = new System.Security.Cryptography.HMACSHA512()){
               passwordsalt=hmac.Key;
               passwordHash=hmac.ComputeHash(System.Text.Encoding.UTF8.GetBytes(password));
           };
        }

        public Task<bool> UserExists(string username)
        {
            throw new System.NotImplementedException();
        }

        /* =================== SQL Area  ======================== */


        public async Task<string> prcRegisterUser(User model)
        {
            CoreSQLConnection CoreSQL = new CoreSQLConnection();
            ArrayList arrayList = new ArrayList();
            var Query = "SELECT  cast(Isnull(MAX(CatId),0) + 1 AS float)  AS CatId FROM tbl_Student";
            var variable = CoreSQL.CoreSQL_GetDoubleData(Query);
            try
            {
                var sqlQuery = "Insert Into tbl_loginUsers (Id, UserName, PasswordHash, PasswordSalt)" +
                               " Values ('" + variable + "','" + model.UserName + "','" + model.PasswordHash + "'," +
                               "'" + model.PasswordSalt + "')";
                arrayList.Add(sqlQuery);
                CoreSQL.CoreSQL_SaveDataUseSQLCommand(arrayList);
                return "Successfully Save.";
            }
            catch (Exception ex)
            {
                throw (ex);
            }
            finally
            {
            }
        }
        public async Task<bool> IsExistUser(string username)
        {
            try
            {
                CoreSQLConnection CoreSQL = new CoreSQLConnection();
                ArrayList arrayList = new ArrayList();
                var Query = "SELECT  cast(Isnull(MAX(UserId),0)AS float)  AS UserId FROM tbl_Student where username='"+username+"'";
                var variable = CoreSQL.CoreSQL_GetDoubleData(Query);
                if(variable>0){
                    return  true;   
                }else{
                return false;
                }
            }
            catch (Exception ex)
            {
                throw (ex);
            }
            finally
            {

            }
        }
    }
}